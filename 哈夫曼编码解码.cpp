...
} header[512],tmp;//一个字节为8位能够存储512种字符
void compress()//压缩函数 
{
    ...
    printf("\t请您输入编码后的文件名：");
    gets(outputfile);
    ofp=fopen(strcat(outputfile,".xhy"),"wb");//只写方式将源文件编译为二进制文件实现压缩
    if(ofp==NULL)
    {
        printf("\n\t编码文件失败!\n\n");
        return;
    ...
    for(i=0;i<512;i++)//将字符按照ASCII码顺序存放入数组中
        {
            if(header[i].count!=0)
                header[i].b=(unsigned char)i;//将每个哈夫曼码值及其对应的ASCII码存放在一维数组header[i]中，编码表中的下标和ASCII码满足顺序存放关系
            else header[i].b=0;
                header[i].parent=-1;header[i].lch=header[i].rch=-1;//对结点进行初始化
        }
    for(i=0;i<256;i++)//根据频率（权值）大小，对结点进行由大到小排序
    {
        for(j=i+1;j<256;j++)
        {
    ...
    for(i=0;i<256;i++)//构建哈夫曼树，选择权值较小的结点进树 
        if(header[i].count==0)
            break;
    ...
    for(i=n;i<m;i++)
    {
        min1=999999999;//挑取未存入树的权值最小的结点作为左孩子，预设的最大权值，即结点出现的最大次数
        for(j=0;j<i;j++)
        {
        ...
        min1=999999999;//挑取未存入树的权值最小的结点作为右孩子
        for(j=0;j<i;j++)
        {
            if(header[j].parent!=-1)
                continue;
    ...
    for(i=0;i<n;i++)//构建哈夫曼编码，哈夫曼无重复前缀编码
    {
        f=i;
    ...
    fseek(ifp,0,SEEK_SET);//压缩文件，定位到文件开始位置
    ...
    if(j>0)//末尾编码不足8位，补0，对哈夫曼编码位操作进行压缩存储
    {
        strcat(buf,"00000000");
        for(i=0;i<8;i++)
        {
...
void uncompress()//解压缩函数 
{
    ...
    printf("\t请您输入需要解码的文件：");
    gets(filename);
    ifp=fopen(strcat(filename,".xhy"),"rb");
    if(ifp==NULL)
    {
        printf("\n\t文件打开失败!\n");
        return;
    ...
    fread(&flength,sizeof(long),1,ifp);//获取原文件信息，得到编码表,读取原文件长度
    ...
    fseek(ifp,8,SEEK_SET);//利用编码表解码,移至正文信息前，（压缩文件前8位用于记录源文件长度）
    m=0;
    bx[0]=0;
